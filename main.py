...
# --- JIOSAAVN MULTI-MIRROR API (100% WORKING & BULLETPROOF) ---
def search_and_get_stream(query: str):
    """
    Multiple live mirrors se check karta hai taaki koi server down ho tab bhi gaana play ho sake.
    """
    ...
    mirrors = [
        ...
        f"https://saavn-api-sigma.vercel.app/api/search/songs?query={encoded_query}",
        f"https://saavn-api-sumit.vercel.app/api/search/songs?query={encoded_query}"
    ]
    ...
    for url in mirrors:
        try:
            ...
            if results and len(results) > 0:
                ...
                stream_url = None
                if "downloadUrl" in track and track["downloadUrl"]:
                    if isinstance(track["downloadUrl"], list):
                        stream_url = track["downloadUrl"][-1].get("url") or track["downloadUrl"][-1].get("link")
                    elif isinstance(track["downloadUrl"], str):
                        stream_url = track["downloadUrl"]
                elif "media_url" in track:
                    stream_url = track["media_url"]

                if stream_url:
                    duration_sec = int(track.get("duration") or 0)
                    dur_str = time.strftime("%M:%S", time.gmtime(duration_sec)) if duration_sec else "03:30"
                    print(f">> Track Found via: {url}")
...
async def play_handler(_, message: Message):
    ...
    # Multi-mirror fallback logic se audio link fetch
