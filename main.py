...
for err_name in ["GroupcallForbidden", "GroupcallInvalid", "GroupcallAlreadyDiscarded"]:
    if not hasattr(pyrogram.errors, err_name):
        err_cls = type(err_name, (Exception,), {})
        setattr(pyrogram.errors, err_name, err_cls)
...
async def play_handler(_, message: Message):
    ...
    try:
        ...
        active_call = call_py if call_py else bot_call
        if active_call:
            await active_call.play(chat_id, MediaStream(stream_url))
...
async def stop_music(_, message: Message):
    ...
    if active_call:
        try:
            await active_call.leave_call(message.chat.id)
...
async def pause_music(_, message: Message):
    ...
    if active_call:
        try:
            await active_call.pause(message.chat.id)
...
async def resume_music(_, message: Message):
    ...
    if active_call:
        try:
            await active_call.resume(message.chat.id)
...
async def callback_handler(_, query: CallbackQuery):
    ...
    if data == "close_panel":
        ...
        if active_call:
            try:
                await active_call.resume(query.message.chat.id)
                await query.answer("Stream Resumed! ▶️")
            except Exception:
                await query.answer("Stream active nahi hai.")
        else:
            await query.answer("Voice engine active nahi hai.", show_alert=True)

    elif data == "pause_stream":
        if active_call:
            try:
                await active_call.pause(query.message.chat.id)
...
async def main():
    ...
    if sess_doc and sess_doc.get("session"):
        ...
        try:
            bot_call = PyTgCalls(bot)
            await bot_call.start()
            print(">> Bot PyTgCalls Started (Default fallback)!")
        except Exception as e:
            print(f"Bot PyTgCalls Note: {e}")
