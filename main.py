...
# --- OFFICIAL JIOSAAVN DIRECT SEARCH & DECRYPT ---
DES_CIPHER_KEY = b"38346591"

def decrypt_saavn_url(encrypted_url: str) -> str:
    try:
        enc_data = b64decode(encrypted_url.strip())
        des = DES.new(DES_CIPHER_KEY, DES.MODE_ECB)
        decrypted = des.decrypt(enc_data)
        # PKCS5 padding remove
        pad = decrypted[-1]
...
def search_and_get_stream(query: str):
    """
    JioSaavn ke official API server se direct query karta hai (No 3rd-party mirrors).
    """
    headers = {
    ...
    try:
        # Step 1: Official JioSaavn Search API
        search_url = (
            f"https://www.jiosaavn.com/api.php?__call=autocomplete.get&_format=json&_marker=0&cc=in&includeMetaTags=1&query={quote(query)}"
        )
        ...
        if not songs:
            print(">> No songs found in search response")
            return None, None, None

        first_song = songs[0]
        song_id = first_song.get("id")
        title = first_song.get("title", "Audio Track").replace("&quot;", '"').replace("&amp;", '&')

        # Step 2: Song Details Fetch karke direct stream URL nikalna
        details_url = (
            f"https://www.jiosaavn.com/api.php?__call=song.getDetails&cc=in&_marker=0%3F_marker%3D0&_format=json&pids={song_id}"
        )
        det_res = requests.get(details_url, headers=headers, timeout=6)
        det_data = det_res.json()

        song_info = det_data.get(song_id)
        if not song_info:
            return None, None, None
        ...
        stream_url = decrypt_saavn_url(encrypted_media_url)
        duration_sec = int(song_info.get("duration", 0))
        dur_str = time.strftime("%M:%S", time.gmtime(duration_sec)) if duration_sec else "03:30"

        if stream_url:
            print(f">> Successfully extracted stream: {title}")
            return stream_url, title, dur_str

    except Exception as e:
        print(f">> Official Saavn API error: {e}")
