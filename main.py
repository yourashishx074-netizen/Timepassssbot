...
# SHRUTIBOTS API KEY
SHRUTI_API_KEY = os.getenv("SHRUTI_API_KEY", "ShrutiBots1IeGY0KqC9XAOckI6qea")
...
# --- SHRUTIBOTS & YOUTUBE STREAM ENGINE ---
def search_and_get_stream(query: str):
    encoded_query = quote(query)
    
    # 1. ShrutiBots API Call
    shruti_endpoints = [
        f"https://api.shrutibots.site/stream?api_key={SHRUTI_API_KEY}&query={encoded_query}",
        f"https://shrutimusic-api.vercel.app/stream?key={SHRUTI_API_KEY}&q={encoded_query}"
    ]

    for api_url in shruti_endpoints:
        try:
            res = requests.get(api_url, timeout=5)
            if res.status_code == 200:
                data = res.json()
                stream_url = data.get("stream_url") or data.get("link") or data.get("url")
                title = data.get("title") or data.get("name") or query
                duration = data.get("duration", "03:45")
                if stream_url:
                    return stream_url, title, duration
        except Exception:
            continue

    # 2. Resilient Direct Backup (In case ShrutiBots server network timeout)
    backup_urls = [
        f"https://jiosaavn-api-privatecvc2.vercel.app/search/songs?query={encoded_query}",
        f"https://saavn.me/api/search/songs?query={encoded_query}"
    ]
    for b_url in backup_urls:
        try:
            r = requests.get(b_url, timeout=4).json()
            results = None
            ...
            if results:
                track = results[0]
                title = track.get("name") or track.get("title", "Audio Track")
                d_urls = track.get("downloadUrl")
                stream_url = None
                if isinstance(d_urls, list) and d_urls:
                    stream_url = d_urls[-1].get("url")
                elif isinstance(d_urls, str):
                    stream_url = d_urls
                
                if stream_url:
                    dur = int(track.get("duration") or 0)
                    dur_str = time.strftime("%M:%S", time.gmtime(dur)) if dur else "03:30"
...
ʏᴏᴜ ᴄᴀɴ ᴄᴏɴᴛʀᴏʟ ᴛʜᴇ ᴘʟᴀʏʙᴀᴄᴋ sᴘᴇᴇᴅ ᴏꜰ ᴛʜᴇ ᴏɴɢᴏɪɴɢ sᴛʀᴇᴀᴍ. [ᴀᴅᴍɪɴs ᴏɴʟʏ]
...
async def play_handler(_, message: Message):
    ...
    if not stream_url:
        await prep_msg.edit(
            "❌ **Track nahi mila! Kripya sahi naam likhein.**"
        )
        return
